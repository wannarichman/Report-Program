...
# --- [ID 동기화 로직: 참여자 고유 번호 유지] ---
def sync_user_id():
    js_code = """
    <script>
    const storageKey = 'posco_uid_v_ultimate_final_fix';
...
with st.sidebar:
    ...
    if is_reporter:
        ...
        if shared_store["report_data"]:
            st.download_button("📥 편집본 JSON 저장", data=json.dumps(shared_store["report_data"], indent=4, ensure_ascii=False), file_name="posco_full_edited.json", mime="application/json", use_container_width=True)
...
def sync_content_area(edit_enabled):
    ...
    with st.expander("💬 실시간 채팅", expanded=True):
        ...
        with i_col:
            ...
            chat_input = st.text_input("채팅 입력", key="chat_in", label_visibility="collapsed")
    ...
    with col_main:
        ...
        if edit_enabled:
            with st.container(border=True):
                img_f = st.file_uploader("🖼️ 본문 이미지 업로드", type=['png', 'jpg'], key=f"img_up_{shared_store['current_page']}")
        ...
        for i, (line, fs) in enumerate(zip(lines, l_fs)):
            if edit_enabled:
                ...
                el = c1.text_input(f"문구 {i+1}", line, key=f"li_{shared_store['current_page']}_{i}")
    ...
    with col_side:
        ...
        if edit_enabled:
            with st.expander("➕ 우측 요소 추가/편집", expanded=True):
                side_img_f = st.file_uploader("🖼️ 우측 이미지 업로드", type=['png', 'jpg'], key=f"s_img_up_{shared_store['current_page']}")
