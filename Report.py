    ...
    if isinstance(raw_data, dict):
        ...
        for k, v in raw_data.items():
            if isinstance(v, (str, int, float)) and len(str(v)) < 50:
                adapted_page["side_blocks"].append({"type": "metric", "label": str(k), "value": str(v)})
            else:
...
def main_content_area(edit_enabled):
    ...
    if is_reporter:
        ...
        if edit_enabled: 
            p['tab'] = st.text_input("🔖 탭 제목 수정", p.get('tab', ''), key=f"tab_edit_{shared_store['current_page']}")
    ...
    with col_main:
        ...
        if edit_enabled and st.button("➕ 하단 상세 블록 추가"):
            p['bottom_blocks'].append({"header": "상세 제목", "header_fs": 28, "content": "상세 내용", "content_fs": 20})
        
        for idx, bb in enumerate(p['bottom_blocks']):
            st.markdown(f'<div class="main-frame">', unsafe_allow_html=True)
            if edit_enabled:
                c1, c2 = st.columns([4, 1])
                bb['header'] = c1.text_input(f"블록 제목 {idx}", bb['header'])
                if c2.button("🗑️", key=f"bdel_{idx}"): p['bottom_blocks'].pop(idx); st.rerun()
                bb['content'] = st.text_area(f"블록 내용 {idx}", bb['content'])
            else:
                st.markdown(f'<h2 style="font-size:{bb.get("header_fs", 28)}px; border-bottom:2px solid #eee;">{bb["header"]}</h2>', unsafe_allow_html=True)
                st.markdown(f'<div class="content-line" style="font-size:{bb.get("content_fs", 20)}px;">{bb["content"]}</div>', unsafe_allow_html=True)
            st.markdown('</div>', unsafe_allow_html=True)

    with col_side:
        st.markdown("<p style='font-weight:bold; color:#666;'>📊 PERFORMANCE</p>", unsafe_allow_html=True)
        p.setdefault('side_blocks', [])
        if edit_enabled and st.button("➕ 사이드 지표 추가"): 
            p['side_blocks'].append({"type": "metric", "label": "신규 지표", "value": "0"})
        
        for idx, block in enumerate(p['side_blocks']):
            st.markdown('<div class="side-frame">', unsafe_allow_html=True)
            # [에러 방지] 키 존재 여부 확인 후 렌더링
            label = block.get("label", "항목")
            value = block.get("value", "0")
            
            if edit_enabled:
                block['label'] = st.text_input(f"라벨 {idx}", label)
                block['value'] = st.text_input(f"값 {idx}", value)
                if st.button(f"🗑️ 지표 {idx}"): p['side_blocks'].pop(idx); st.rerun()
            
            st.markdown(f'<small>{label}</small><div style="font-size:26px; font-weight:800; color:#007bff;">{value}</div>', unsafe_allow_html=True)
            st.markdown('</div>', unsafe_allow_html=True)
