...
import base64
import os
import urllib.parse  # 자동 이미지 검색어(image_query) URL 인코딩용
...
def get_sample_json_guide():
	return {
		...
		"pages": [
			{
				"tab": "요약",  # P1. 중복 출력 방지를 위해 순수 탭 이름만 유지
...
def adapt_json_format(raw_data):
	# 최소한 pages만 있으면 통과시키되, 전체 제목 등의 기본값을 보정
...
# 자동 이미지 URL 생성 (안정적인 Picsum API 적용)
...
# 이미지 소스 렌더링 헬퍼 (웹URL, Base64, 로컬파일 모두 지원)
...
# ==========================================
# 4. ID 식별 및 음성 시스템 (UID persistence 및 Heartbeat)
# ==========================================
...
def agora_voice_system(app_id, channel, user_label):
	# [치명적 버그 수정 완료] JS 객체 및 화살표 함수에 이중 중괄호 {{ }} 적용
	custom_html = f"""
...
def main_content_area(edit_enabled):
	...
	if is_reporter:
		# [수정] 코드 상에서 "P{i+1}. "를 자동으로 붙여주므로, JSON의 tab 이름은 순수 텍스트만 유지합니다.
	...
	# --- 6-6. 섹션 (블록) 루프 (Key Collision 해결 로직) ---
	...
	for s_idx, sec in enumerate(sections):
		with st.container(border=True):
			...
			with col_main:
				# --------------------------
				# 좌측 메인 이미지 관리 (URL + 업로드 + image_query 자동)
				# --------------------------
				if edit_enabled:
