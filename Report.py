...
with st.sidebar:
    ...
    # 편집 모드 활성화
    edit_mode = st.toggle("📝 편집 및 가독성 설정 모드", value=False)
        ...
        with tab:
            ...
            with col_main:
                if edit_mode:
                    ...
                    # 그림 크기 조절용 슬라이더 추가 (드래그 대신 정밀 조절)
                ...
                if "image" in p:
                    # 슬라이더 값에 따라 그림 크기 동적 반영
                    st.image(p["image"], width=int(img_width * 10)) # 100%일 때 약 1000px
                
                # [핵심] 본문 글자 크기를 키워서 출력 (Markdown 활용)
                st.markdown(f"### {p.get('content', '').replace('\\n', '  \n')}")
