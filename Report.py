                ...
                "sections": [
                    {
                        ...
                        "side_items": [
                            {"type": "metric", "label": "진행률", "value": "0%", "color": "#007bff", "label_fs": 14, "label_color": "#64748b", "value_fs": 34},
                ...
                "sections": [
                    {
                        ...
                        "side_items": [
                            {"type": "metric", "label": "Blocker", "value": "없음", "color": "#dc2626", "label_fs": 14, "label_color": "#64748b", "value_fs": 26}
                        ],
                    }
                ],
            },
...
def main_content_area(edit_enabled):
    ...
    ERROR_IMG = "https://placehold.co/800x400/f8fafc/94a3b8?text=Image+Not+Found+or+Invalid+URL"
    ...
    for s_idx, sec in enumerate(sections):
        with st.container(border=True):
            if edit_enabled:
                ...
                if sc5.button("🗑️", key=f"st_del_{cp_idx}_{s_idx}"):
                    sections.pop(s_idx)
                    st.rerun()
            ...
            with col_main:
                if edit_enabled:
                    with st.expander("🖼️ 이미지 관리"):
                        ...
                        if not sec["full_width"]:
                            sec["img_width"] = st.slider("수동 너비 조절", 100, 1200, int(sec.get("img_width", 750)), key=f"sw_{cp_idx}_{s_idx}")

                        if st.button("🗑️ 그림 삭제", key=f"simg_del_{cp_idx}_{s_idx}"):
                            sec["main_image"] = None
                            st.rerun()
                ...
                if edit_enabled:
                    ...
                    for l_idx, line in enumerate(sec["lines"]):
                        ...
                        if not lc4.button("🗑️", key=f"lt_del_{cp_idx}_{s_idx}_{l_idx}"):
                            new_lines.append({"text": l_t, "size": l_s, "color": l_c})
                    sec["lines"] = new_lines

                    if st.button("➕ 문구 줄 추가", key=f"lt_add_{cp_idx}_{s_idx}"):
                        ...
                        with st.expander(f"⚙️ {item.get('label', '아이템')} 편집", expanded=True):
                            if item["type"] == "metric":
                                ...
                                item["image_query"] = st.text_input(
                                    ...
                                    key=f"si_q_{cp_idx}_{s_idx}_{i_idx}",
                                )
                            ...
                            if st.button("🗑️ 삭제", key=f"si_del_{cp_idx}_{s_idx}_{i_idx}"):
